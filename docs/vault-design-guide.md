...
│   ├── Today.md        # Daily entry point (generated by EOD, read each morning)
│   ├── ClientA.md      # Per-client task files
│   ├── ClientB.md
│   ├── [YourCompany].md # Cross-client / agency / hiring / internal tasks
...
- **`Today.md`** -- The daily entry point. Generated by the EOD pipeline and contains the schedule, prioritized tasks, meeting prep, deadline radar, and strategic goals. Read each morning, then reviewed interactively via `/morning`. There is no separate overview dashboard; the per-client files below are the source of truth.
- **One file per client** (e.g., `ClientA.md`, `ClientB.md`) -- Each follows a standard structure:
- **`[YourCompany].md`** -- Holds cross-client, agency, hiring, and internal tasks. Use the same structure as a client file.
- **`Personal.md`** -- Personal tasks, same structure.
...
> Tasks are flat bullets. Source context lives in the italic suffix, not in subsection headers. Do not group tasks under `### New from <source>` subheaders -- it bloats the file as history accumulates.
...
### Notes from <source> MM/DD
- Meeting-boundary grouping is fine here; notes benefit from source context
